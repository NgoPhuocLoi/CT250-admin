...
<template>
    <div class="card">
        <TableSkeleton v-if="fetchingProducts" />
        <DataTable v-else stripedRows v-model:filters="filters" :value="products" filterDisplay="row"
            :loading="fetchingProducts" :globalFilterFields="['id', 'name', 'rootCategory', 'visible']"
            sortMode="multiple" paginator resizableColumns columnResizeMode="fit"
            paginatorTemplate="FirstPageLink PrevPageLink CurrentPageReport NextPageLink LastPageLink RowsPerPageDropdown"
            currentPageReportTemplate="{first} đến {last} trong tổng số {totalRecords}" :rows="10"
