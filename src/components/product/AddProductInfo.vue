  ...
  <div class="w-[90%] flex flex-col gap-6">
    <div class="flex flex-col gap-6">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="card">
          <CascadeSelect v-model="productStore.selectedCategory" :options="categoryStore.categories" optionLabel="name"
            optionGroupLabel="name" :optionGroupChildren="['children', 'children']" style="min-width: 14rem"
      ...
      <div class="flex flex-col gap-2">
        ...
        <div>
          <input class="mr-2" v-model="productStore.newProduct.visible" type="radio" id="visible" name="visibility"
            value="true" />
          <label for="visible"> Hiển thị</label><br />
          <input class="mr-2" v-model="productStore.newProduct.visible" type="radio" id="invisible" name="visibility"
            value="false" />
    ...
    <div class="flex flex-col gap-2">
      ...
      <div class="grid grid-cols-4 gap-2 border rounded p-4">
        <div v-for="(image, index) of productStore.images" :key="image.file.name"
          ...
          <img class="w-full object-contain overflow-hidden" :src="image.path" />
        ...
        <div @click="productStore.uploadImage('productImages')"
          ...
          <ImageIcon />
          <div class="absolute top-0 left-0 invisible">
            <input accept="image/*" type="file" id="productImages" @change="productStore.addProductImage" multiple />
          </div>
        </div>
      </div>
    </div>
